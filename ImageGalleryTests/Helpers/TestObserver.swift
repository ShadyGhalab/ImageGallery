...
public final class TestObserver <Value, Error: Swift.Error> {
    ...
    public var values: [Value] {
        return self.events.filter { $0.isValue }
            .map { $0.value! }
    }
    ...
    public var failedError: Error? {
        return self.events.filter { $0.isFailed }
            .map { $0.error! }.first
    }
    ...
    public var didComplete: Bool {
        return !self.events.filter { $0.isCompleted }.isEmpty
    }
    
    /// `true` if a .Interrupt` event has been emitted.
    public var didInterrupt: Bool {
        return !self.events.filter { $0.isInterrupted }.isEmpty
    }
